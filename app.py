...
app.config['UPLOAD_IMAGE_solar'] = UPLOAD_IMAGE_solar
app.config['UPLOAD_IMAGE_solar_detect'] = UPLOAD_IMAGE_solar_detect
...
def garuda_solar():
    return render_template('solar_select.html')
...
def garuda_solar_detect():
    return render_template('solar_detect.html')
...
def garuda_solar_detect_image():
    ...
    if file and allowed_file(file.filename):
        filename = secure_filename(file.filename)
        file.save(os.path.join(app.config['UPLOAD_IMAGE_solar'], filename))
        #print('upload_image filename: ' + filename)
        TEST_IMAGE = UPLOAD_IMAGE_solar + filename    
        print("TEST_IMAGE" , TEST_IMAGE)
        DETECTION_URL = "http://127.0.0.52:5000/v1/solar-detection/fastrcnn"
        #http://127.11.0.52:5000/v1/solar-detection/fastrcnn?name=DJI_0096.JPG
        params = {'name': filename}
        response = requests.get(DETECTION_URL,params=params)
        print(response.url)        
        # requests.post(url = DETECTION_URL, data = data1) 
        # requests.post(DETECTION_URL, json={"name": image})
        RES_NAME = UPLOAD_IMAGE_solar_detect + filename
        return send_file(RES_NAME, mimetype='image/gif')
        # return 'file uploaded successfully'
    
@app.route('/solar_display/<filename>')
def display_solar_image(filename):
    print('display_image filename: ' + filename)
    return redirect(url_for('UPLOAD_IMAGE_solar_detect', filename= filename), code=301)
...
def label():
    # jupyter_path = 'C:/Program Files/Google/Chrome/Application/chrome.exe %s'
    # webbrowser.get(jupyter_path).open_new("www.google.com")
    os.chdir(r'D:/AI_MODEL/SITE/Annotation_tool/yolo_annotation_tool')
    os.system("python labelImg.py")
    return render_template('page-3.html')

@app.route("/label_poly")
def label_poly():
    os.system("labelme")
    return render_template('page-3.html')
